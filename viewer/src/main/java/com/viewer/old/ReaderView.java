...
public class ReaderView
	...
	public void applyToChildArround(ViewMapper mapper, Integer position) {
		for (int i = 0; i < mChildViews.size(); i++) {
			Log.d("applyingThingsToChild", "searching for pos "+position+". this is "+((PageView) mChildViews.valueAt(i)).getPage());
			Integer pos = ((PageView) mChildViews.valueAt(i)).getPage();
			if (pos == position || pos + 1 == position || pos-1 == position) {
				Log.d("applyingThingsToChild", "go!!!");
				mapper.applyToView(mChildViews.valueAt(i));
			}
		}
	}

	public void applyToChild(ViewMapper mapper, Integer position) {
		for (int i = 0; i < mChildViews.size(); i++) {
			Integer pos = ((PageView) mChildViews.valueAt(i)).getPage();
			if (pos == position) {
				mapper.applyToView(mChildViews.valueAt(i));
